...
from models import KGEModel, RotatE, DMMCS
...
def parse_args(args=None):
    ...
    parser.add_argument('--data_path', type=str, default='F:\\tian\\DMMCS\\data\\YAGO3-10')
    parser.add_argument('--model', default='DMMCS', type=str)
    parser.add_argument('--regularization', default=0, type=float)
    parser.add_argument('-n', '--negative_sample_size', default=400, type=int)
    parser.add_argument('-d', '--hidden_dim', default=400, type=int)
    parser.add_argument('-g', '--gamma', default=18.0, type=float)
    ...
    parser.add_argument('-save', '--save_path', default='F:\\tian\\DMMCS\\save\\yago3-10\\new11_18_2', type=str)
    ...
    # 不指定则两次下滑学习率
    parser.add_argument('--one_decay', action='store_true', help='Learning rate do not decay')
...
def main(args):
    ...
    elif args.model == 'DMMCS':
        kge_model = DMMCS(num_entity, num_relation, args.hidden_dim, args.gamma)

    logging.info('Model Parameter Configuration:')
    for name, param in kge_model.named_parameters():
        logging.info('Parameter %s: %s, require_grad = %s' % (name, str(param.size()), str(param.requires_grad)))

    kge_model = kge_model.cuda(0)
    ...
    if args.do_train:
        ...
        for step in range(init_step, args.max_steps):
            ...
            if step >= warm_up_steps and number_of_decay == 0:
                if not args.one_decay:
            ...
            elif step >= warm_up_steps and number_of_decay == 1:
                if not args.one_decay:
