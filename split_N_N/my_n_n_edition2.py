...
# 数据的上层目录名
data_root = 'E:\\thesis_for_me\\thesis_codes\\DMMCS\\data\\wn18'
train_path = os.path.join(data_root, 'train.txt')
valid_path = os.path.join(data_root, 'valid.txt')
test_path = os.path.join(data_root, 'test.txt')

all_path = [train_path, valid_path, test_path]

for path in all_path:
    with open(path, "r", encoding='utf-8') as triple_in:
...
snn = 0
num_of_test = 0
with open(train_path, 'r', encoding='utf-8') as test_in:
    for line in test_in:
        num_of_test += 1
        line = line.strip().split('\t')
        if len(line) != 3:
...
# 保存各关系的上层目录名
save_root = 'E:\\thesis_for_me\\thesis_codes\\rotate\\KnowledgeGraphEmbedding-master\\data\\wn18'
rootall = os.path.join(save_root, "wn18rrall.txt")
root11 = os.path.join(save_root, "1-1.txt")
root1n = os.path.join(save_root, "1-n.txt")
rootn1 = os.path.join(save_root, "n-1.txt")
rootnn = os.path.join(save_root, "n-n.txt")
